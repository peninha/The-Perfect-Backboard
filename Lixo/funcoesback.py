# -*- coding: utf-8 -*-
"""
Created on Thu Aug 18 10:08:00 2022
...
def colisor(P, Vx, Dots, Angles, maxDotsX):
    ...
    #if P[0] <= maxDotsX+bolaRaio and P[0] >= bolaRaio/2:
...
def shoot(P0, V0, timeRange=2, rebatida=False, Dots=None, Angles=None, maxDotsX=None, grafico=False, size=1, teste=False):
    global deltaT, aroP
    grid = 0
    X = []
    Y = []
...
def grid2Coords(i, j):
    ...
    #z = (j - (gridSizeY-1)/2) * tabelaLargura/(gridSizeY-1) + focoAltura + aroAltura
    z = (j - (gridSizeY-1)/2) * tabelaLargura/(gridSizeY-1) + focoAltura + aroAltura + 50
    x = 0
...
def preparaPlot(visao=1):
    ######### Configura o PyPlot ##########
    global fig, ax, aroP, aro2P, tabelaP, tabela2P
    fig = plt.figure()
